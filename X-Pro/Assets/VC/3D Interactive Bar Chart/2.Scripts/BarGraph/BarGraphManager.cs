...
namespace BarGraph.VittorCloud
{
    public class BarGraphManager : GraphBox
    {
        ...
        public void SetGraphData(List<BarGraphDataSet> dataSet)
        {
        ...
        public IEnumerator GenerateGraphBarWithAnimTypeOne(int xIndex, int zIndex, float yValue, float scaleFactor, float animSpeed, int ymin, int xMax, Color barColor)
        {
            ...
            SetBarProperties(barProperty,xIndex,zIndex);
        ...
        public IEnumerator AnimateBarsWithAnimTypeOne(BarProperty bar, float barScale, float yValue, float animSpeed, bool isUpdating)
        {
            while (true)
            {
                ...
                if (bar.transform.localScale.y > targetScale.y)
                {
                    ...
                    Debug.Log($"y scale {scale.y} target scale {targetScale.y} yvalue {yValue}");
        ...
        public void SetBarProperties(BarProperty barProperty, int x_index, int z_index)
        {
            barProperty.dataSet = new BarDataSet(barProperty.gameObject,x_index,z_index,dataSet[z_index].ListOfBars[x_index].XValue
                , dataSet[z_index].ListOfBars[x_index].YValue
                , dataSet[z_index].GroupName
                );
